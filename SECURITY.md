...
- PARTITION 的逐记录执行权和成功业务的确认责任属于当前进程与来源代次。读取前冻结的权威沿 Task、
  重试与确认链传递，失效快照不能借新的 holder 或代次执行、确认；这些状态不构成跨进程持久业务账本，
  接管或重启后仍须使用稳定事件键去重。
- source/group/stream 只划分本地执行与数量预算，不构成租户安全隔离、内存字节隔离或外部系统事务边界。
  同一计划同 key 的顺序门禁跨域共享，普通 PROXY 与 PARTITION 之间不提供顺序或确认协调。
- 停机超时可以结束 Claim 的持权等待，但不能把执行中的 ACK/PEL、恢复或业务副作用当作已经结束。
  本地注册与容量保留到实际返回和清理，迟到外部副作用仍由业务幂等约束。
